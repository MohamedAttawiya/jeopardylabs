...
if (screens.config && rowsInput && colsInput) {
  ...
  function isConfigComplete() {
    ...
    const names = Array.from(categoryInputs).map((input) => input.value.trim());
    const filledCategories = categoryInputs.length === cols && names.every((name) => name.length > 0);
    const duplicates = hasDuplicateCategories(names.filter(Boolean));

    return Boolean(title && rows && cols && filledCategories && !duplicates);
  }
  ...
  function syncGridValidity() {
    ...
    gridError.textContent = complete ? '' : 'Fill every question and answer before continuing.';
  }
  ...
  function bindEvents() {
    ...
    gridContainer.addEventListener('keyup', (event) => {
      if (event.target.matches('[data-cell-open]')) {
        syncGridValidity();
      }
    });
